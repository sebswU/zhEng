...
from voyager.models import Person,TxtRec, WordCard
from voyager.forms import inputForm
...
def view(request):
    ...
    #TODO: if project is up for OS dev, all vars must be configurable
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('voyager:archive'))           

    return render(request,'index.html',{'form':form, 'user':request.user.username})
...
class WordCardView(ListView):
    model = WordCard
    template_name="wordcard_list.html"
